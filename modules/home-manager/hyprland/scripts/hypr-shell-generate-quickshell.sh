...
EOF
	...
	# GeneratedTheme.qml is generated by home-manager from local.theme options.
	# Prefer the generated copy so TTY tests use the same tokens as the session.
	hm_theme="${XDG_CONFIG_HOME:-$HOME/.config}/quickshell/hyprland/GeneratedTheme.qml"
	if [ -f "$hm_theme" ]; then
		cp "$hm_theme" "$output_dir/GeneratedTheme.qml"
	fi
fi
