...
//
// Most bar items share the same height, rounded corners, hover color, and
// active color. Keeping that frame here means future changes to the bar's
// basic shape only need one edit.
Rectangle {
    id: root
    ...
    readonly property Gradient accentFill: Gradient {
        ...
        GradientStop { position: 0.0; color: Theme.barAccentGradientStart }
        GradientStop { position: 1.0; color: Theme.barAccentGradientEnd }
    }

    height: Theme.barControlHeight
    radius: Theme.barControlRadius
    color: Theme.barControlColor(root.active, root.hoverActive, root.urgent)
